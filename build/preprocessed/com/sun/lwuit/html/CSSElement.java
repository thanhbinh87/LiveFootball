...
class CSSElement extends Element {
    ...
    private static final String[] BORDER_STYLE_STRINGS = {CSS_NONE,"solid","groove","ridge","inset","outset","dotted","dashed","double"};
    ...
    static final int CSS_TEXT_DECORATION = CSS_STYLE_ID_OFFSET + 44; // Not implemented, since the only mandatory WCSS decoration value is 'none' which is usually used to remove underlines from links - since we don't have underlines it has no meaning
    ...
    */
     ...
     * @param fontHeight The font height of the current element
    ...
    static int convertUnits(String units) {
        ...
        int factors[] = {1,2,1,DPI,DPI/72,DPI/6,(int)(DPI/2.54/10),(int)(DPI/2.54)}; // unit factors for {"px","em","ex","in","pt","pc","mm","cm"};
    ...
    /**
     * Construvts a CSSElement, This basically sets the name and ID and resets the attrVals array.
     * 
     * @param name
     */
    ...
    int calcSelectorSpecificity() {
        ...
        if ((selectorPseudoClass!=0) && (nameStr.equals(""))) { // :link is the same as a:link
            nameStr="a";
        }
        ...
        if (index!=-1) {
            spec+=100;
            selectorId=name.substring(index+1);
            if (index!=0) {
                spec+=1;
                selectorTag=name.substring(0, index);
            }
        } else {
            index=nameStr.indexOf('.');
            if (index!=-1) {
                spec+=10;
                selectorClass=name.substring(index+1);
